...
const MAX_TICK_DURATION_MS = 20000;
...
export async function manualTick(): Promise<{ advanced: number; signals: number }> {
  const signals = await fireDueMarketSignalsCount();
  const active = await db.execution.findMany({
    where: { status: { notIn: [...TERMINAL_STATES] } },
    select: { id: true },
  });
...
// On-demand advancement for serverless (Vercel).
// The background setInterval ticker only persists in a long-running server
// (dev). On Vercel, function invocations are short-lived, so we advance active
// executions opportunistically on each polled read. Because the UI polls every
// 2s, the state machine still progresses smoothly. This is a no-op if the
// dwell time hasn't elapsed, so it's safe to call on every request.
export async function advanceActiveExecutions(limit = 20): Promise<void> {
  try {
    await fireDueMarketSignals();
    const active = await db.execution.findMany({
      where: { status: { notIn: [...TERMINAL_STATES] } },
    ...
    for (const e of active) {
      try {
        ...
        console.error(`[dRamp engine] on-demand advance error for ${e.id}:`, err);
      }
    }
  } catch (err) {
    console.error("[dRamp engine] on-demand advance error:", err);
  }
}

// Advance a single execution on-demand (used by intent detail polling so the
// golden demo progresses even without the background ticker).
export async function advanceOneOnDemand(executionId: string): Promise<void> {
  try {
    await fireDueMarketSignals();
    await advanceExecution(executionId);
  } catch (err) {
    console.error(`[dRamp engine] on-demand advance error for ${executionId}:`, err);
  }
}

async function fireDueMarketSignalsCount(): Promise<number> {
  const now = new Date();
  const due = await db.marketSignal.findMany({
    where: { fired: false, triggerAt: { lte: now } },
    take: 10,
